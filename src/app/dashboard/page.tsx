...
type DashboardData = {
  ...
  plan: null | { opportunities: Opportunity[] };
  updatedAt: string | null;
  completeness: number;
};
type Action = { id: string; opportunity: Opportunity; status: string; note: string; updatedAt: string; completedAt: string | null };
...
export default function Dashboard() {
  ...
  async function refresh() {
    const [dashboardResponse, actionResponse] = await Promise.all([fetch('/api/dashboard'), fetch('/api/actions')]);
    const dashboard = await dashboardResponse.json();
    const actionData = actionResponse.ok ? await actionResponse.json() : { actions: [] };
    setData(dashboard);
    setSavedActions(actionData.actions ?? []);
  }

  useEffect(() => { refresh().catch(() => setData(null)); }, []);

  const profileComplete = Boolean(data?.profile);
  const metrics = data?.metrics;
  const opportunities = data?.plan?.opportunities ?? [];
  const tracked = savedActions.filter((a) => a.status !== 'dismissed');
  const actions = tracked.length > 0
    ? tracked.slice(0, 3).map((a, i) => [String(i + 1), a.opportunity.title, a.opportunity.nextStep, '/plan'])
    : profileComplete
