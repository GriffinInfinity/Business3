...
export default function PlanPage() {
  ...
  async function submit(event: FormEvent) {
    ...
    setStatuses(Object.fromEntries((existing.actions ?? []).map((action: { opportunity: Opportunity; status: ActionStatus }) => [action.opportunity.id, action.status])));
  }

  async function setAction(opportunity: Opportunity, status: ActionStatus) {
    setSaving(opportunity.id); setError('');
    const response = await fetch('/api/actions', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ opportunity, status }) });
    const data = await response.json();
